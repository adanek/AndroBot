...
public class BlobActivity extends Activity {
	...
	private static Scalar GREEN = new Scalar(88.0, 240.0, 47.0, 0.0);
	private static Scalar RED = new Scalar(0.0, 255.0, 105.0, 0.0);

	private static final String LOG_TAG = "BlobActivity";
	private static Scalar color = GREEN;
	...
	private static BlobDetection prog;
	...
	private void onRunning() {
		...
		if (ball == null) {
			...
			Log.d(LOG_TAG, String.format("Position: %d %d %d", prog.getCurrent().getX(), prog.getCurrent().getY(), prog
					.getCurrent().getTh()));

			// Catch the ball
			int ang = prog.getAngle(new Position(), new Position(x, y, 0));
			prog.turn(ang);
			Log.d(LOG_TAG, "I have to turn " + ang);

			int dis = prog.getDistanceToTarget(new Position(), new Position(x, y, 0)) - 15; // 15 cm vor dem Ball stehen
																							// bleiben.
			prog.moveDistance(dis);
			Log.d(LOG_TAG, "I#ve to move " + dis);
