...
public abstract class ProgrammBase {
	...
	public ProgrammBase(Context context) {
		...
		SharedPreferences settings = context.getSharedPreferences(
				"Androbot_Settings", android.content.Context.MODE_PRIVATE);
		...
		this.robot.setAngularCorrection(settings.getFloat(
				SettingsActivity.ANGULAR_CORRECTION, 2.0f));
		this.robot.setAngularRuntimePerDegree(21.5);

		// Set the angular calibration
		this.robot.setLinearCorrection(settings.getFloat(
				SettingsActivity.LINEAR_CORRECTION, 0.5f));
	...
	public void turn(int degrees) {
		...
		int runtime = (int) (Math.abs(degrees) * robot
				.getAngularRuntimePerDegree());
	...
	public int getAngle(Position current, Position target) {
		int total = (int) Math
				.toDegrees(Math.atan2(target.getY() - current.getY(),
